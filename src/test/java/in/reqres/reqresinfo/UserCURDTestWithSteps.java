...
public class UserCURDTestWithSteps extends TestBase {
    ...
    static String name = "Tom" + TestUtils.getRandomValue();
    static String job = "API Tester" + TestUtils.getRandomValue();
    static String email = "tom.eve@reqres.in";
    ...
    @Title("3. This will if verify second data id = 12")
    @Test
    public void test009() {
        ValidatableResponse response = usersSteps.getAllUserFromPageTwo();
        int data = response.log().all().extract().path("data[1].id");
        Assert.assertEquals(12, data);
    }

    @Title("4. This will if verify forth data first_name = Reena")
    @Test
    public void test010() {
        ValidatableResponse response = usersSteps.getAllUserFromPageTwo();
        String firstname = response.log().all().extract().path("data[3].first_name");
        Assert.assertEquals("Reena", firstname);
    }
